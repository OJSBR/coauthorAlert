...
4. Add/keep translation strings in `locale/<lang>/locale.po` (`en` is the master file; every locale must carry exactly the same keys).
...
`stable-3_5_0` (OJS 3.5.x). **Baseie seu trabalho — e
...
4. Mantenha as strings de tradução em `locale/<idioma>/locale.po` (`en` é o arquivo-mestre; todo idioma precisa ter exatamente as mesmas chaves).
